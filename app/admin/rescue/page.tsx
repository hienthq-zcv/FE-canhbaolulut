...
export default function AdminRescuePage() {
  ...
  const deleteRequest = async (id: number) => {
    if (
      confirm("Bạn có chắc chắn muốn xóa yêu cầu cứu trợ này khỏi Database?")
    ) {
      try {
                ...
                <div
                  ...
                  className={`p-4 rounded-lg border shadow-sm transition-all bg-background ${
                    sos.status === "pending"
                      ...
                      : sos.status === "processing"
                      ? "border-l-4 border-l-yellow-500 border-gray-200 dark:border-yellow-800"
                      : "border-l-4 border-l-green-500 border-gray-200 dark:border-green-800 opacity-60"
                  }`}
                >
                  ...
                  <div className="text-sm space-y-1 mb-3 text-foreground">
                    ...
                    {sos.message && (
                      <div className="text-xs italic bg-gray-50 dark:bg-gray-900 p-2 rounded border border-gray-300 dark:border-gray-700 mt-2 text-red-700 dark:text-red-400 font-medium">
                        "{sos.message}"
                      </div>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-2 border-t border-gray-200 dark:border-gray-700 mt-2">
                    ...
                    {sos.status === "pending" && (
                      <Button
                        ...
                        onClick={() => updateStatus(sos.id, "processing")}
                      >
                    ...
                    {sos.status === "processing" && (
                      <Button
                        ...
                        onClick={() => updateStatus(sos.id, "resolved")}
                      >
                    ...
                    <Button
                      ...
                      className={`h-8 text-xs ${
                        sos.status === "resolved" ? "col-span-2" : ""
                      }`}
                      onClick={() => deleteRequest(sos.id)}
                    >
