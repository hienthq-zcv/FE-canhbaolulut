...
export default function CommunityPage() {
  ...
  // Check if admin and redirect to admin area
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-muted p-4 pt-24">
        ...
        <div className="flex flex-col gap-4 mb-2">
          ...
          <div className="relative">
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-border focus:outline-none focus:ring-2 focus:ring-primary shadow-sm bg-card text-foreground"
              value={searchQuery}
        ...
        <div className="bg-card rounded-xl shadow-sm border border-border p-4">
          <div className="flex gap-3">
            ...
            <div className="flex-1 space-y-3">
              <textarea
                className="w-full bg-muted rounded-lg p-3 focus:outline-none focus:ring-1 focus:ring-primary min-h-[80px] text-foreground placeholder-muted-foreground resize-none"
                placeholder={`${
                  currentUser?.full_name || "Bạn"
                } ơi, hãy chia sẻ tình hình nhé...`}
                value={content}
              ...
              <div className="relative flex items-center">
                ...
                <input
                  type="text"
                  className="w-full pl-10 pr-4 py-2 bg-muted rounded-lg border border-transparent focus:bg-card focus:border-primary/50 focus:outline-none text-sm transition-all text-foreground"
          ...
          {previewUrl && (
            <div className="relative mt-3 ml-12 rounded-lg overflow-hidden bg-muted border border-border">
              <img
                src={previewUrl}
                alt="Preview"
              ...
              <button
                ...
                className="absolute top-2 right-2 bg-foreground/70 dark:bg-background/70 hover:bg-foreground dark:hover:bg-background text-background dark:text-foreground rounded-full p-1.5 transition"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
          ...
          <div className="flex justify-between items-center mt-3 pt-3 border-t border-border ml-12">
            <button
              onClick={() => fileInputRef.current?.click()}
              className="flex items-center gap-2 text-muted-foreground hover:bg-muted px-3 py-2 rounded-lg transition text-sm font-medium"
            >
              <ImageIcon />
              <span>Ảnh/Video</span>
            </button>
            ...
            <button
              ...
              className="bg-primary text-primary-foreground px-8 py-2 rounded-lg hover:bg-primary/90 disabled:bg-primary/50 disabled:cursor-not-allowed transition font-semibold shadow-sm"
            >
        ...
        <div className="space-y-4">
          {filteredPosts.map((post) => {
            ...
            return (
              <div
                key={post.id}
                className="bg-card rounded-xl shadow-sm border border-border overflow-hidden animate-in fade-in duration-300"
              >
                <div className="p-4 flex items-start gap-3">
                  <div className="w-10 h-10 bg-gradient-to-br from-primary to-primary/80 dark:from-primary/90 dark:to-primary/70 rounded-full flex items-center justify-center text-primary-foreground font-bold shadow-sm">
                ...
                {post.image_url && (
                  <div className="mt-2 w-full bg-muted border-y border-border">
                    <img
                      src={post.image_url}
                      alt="Post content"
                  ...
                  <div className="flex items-center gap-1">
                    {isLiked && (
                      <div className="p-1 bg-destructive rounded-full">
                        <svg
                          className="w-2 h-2 text-white dark:text-background"
                          fill="currentColor"
                          viewBox="0 0 24 24"
                        >
                    ...
                    <span>
                      {isLiked ? (post.likes || 0) + 1 : post.likes || 0} người
                      thích
                    </span>
                  </div>
                  <div
                ...
                <div className="px-2 py-1 flex items-center justify-between border-b border-border">
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg transition hover:bg-muted ${
                      isLiked
                  ...
                  <button className="flex-1 flex items-center justify-center gap-2 py-2 rounded-lg transition hover:bg-muted text-muted-foreground">
                    <ShareIcon />
                    <span>Chia sẻ</span>
                  </button>
                </div>
                ...
                {/* --- KHUNG BÌNH LUẬN --- */}
                {isCommentOpen && (
                  <div className="bg-muted p-4 animate-in slide-in-from-top-2 duration-200">
                    <div className="flex gap-2 mb-4">
                      <div className="w-8 h-8 rounded-full bg-primary/20 dark:bg-primary/30 flex items-center justify-center text-xs font-bold text-primary flex-shrink-0">
                        {currentUser?.full_name
                          ? currentUser.full_name[0].toUpperCase()
                          : "me"}
                      </div>
                      <div className="flex-1 relative">
                        <input
                          type="text"
                          className="w-full rounded-full border border-border bg-card py-2 px-4 pr-10 focus:outline-none focus:border-primary text-sm text-foreground"
                        ...
                        <button
                          ...
                          className="absolute right-2 top-1.5 hover:bg-muted p-1 rounded-full transition"
                        >
                          <SendIcon />
                        </button>
                      </div>
                    ...
                    <div className="space-y-3">
                      {[...postComments]
                        ...
                        <div key={comment.id} className="flex gap-2 group">
                          <div className="w-8 h-8 rounded-full bg-muted dark:bg-muted/50 flex items-center justify-center text-xs font-bold text-muted-foreground flex-shrink-0">
                            {comment.user_name
                              ? comment.user_name[0].toUpperCase()
                              : "U"}
                          </div>
                          <div className="flex flex-col">
                            <div className="bg-card p-2.5 rounded-2xl rounded-tl-none shadow-sm border border-border inline-block">
